...
| `TagFilter` | 首屏下方 | 未选 / 选中（`aria-current="true"`） | 服务端 `<Link>`：点标签 → `/?tag=…`；已选中时同一标签的链接指向 `/`（再点即取消）；手机横向滚动、桌面换行；**零客户端 JS** | 「按标签逛」「科幻 2」「冒险 1」「武侠 1」「悬疑 1」「都市 1」 |
...
| 标签条 | 与首页同款；选中 chip 用主色软底 + 主色文字 + 字重 600 + `aria-current="true"` |
...
实现约定：卡片、列表条目与**标签**都是原生 `<a>`（Enter 进入）。标签用服务端渲染的 `<Link>` + `aria-current="true"`（D-013）：点击导航到 `/?tag=…`，已选中时该链接指向 `/`，因此"再点取消"是纯 URL 行为，不需要客户端组件（`design.md` §5 的客户端组件预算保持 3 个）。不要把 `div` 当按钮。
...
| 选中态的表意 | 字重 600 + 主色底 + 主色描边 + `aria-current="true"` | ✅ 不只靠颜色（1.4.1） |
...
10. **键盘与语义**：原生 `button`/`a`；标签用 `<a>` + `aria-current="true"`（D-013，服务端可点、零 JS）；当前导航用 `aria-current="page"`；图片 `alt` 规则 —— 封面「封面：{标题}」，页图「第 x 话 第 y 页」。
...
| v1.0.2 | 2026-09-25 | 按架构师裁决 **D-013**：标签筛选由「客户端 `<button aria-pressed>`」改为「服务端 `<Link>` + `aria-current="true"`」（零 JS、URL 驱动、可中键新窗口打开）；同步 §5.2、§7.4、§7.5 与 §10.10。 |
